...
public class GunMultiplayer : MonoBehaviour {
    ...
    void Start()
    {
        ...
        suckBar = GameObject.Find("playerUI/suckBar").GetComponent<Image>();
        Debug.Log(suckBar);
    }
    ...
    void LateUpdate()
    {
        ...
        if (_gunActive)
        {
            ...
            if (Physics.Raycast(transform.position, fwd, out hitInfo, maxDist, layerMask) && suckAmount > 10)
            {
                ...
                Transform _transform = hitInfo.transform;

                _transform.gameObject.GetComponent<Ghost.GhostMovementMultiplayer>().Suck(ghostToGunVec, _attractionSpeed);
                
                // If the ghost reaches the gun, destroy it
                if (Vector3.Distance(hitInfo.transform.position, transform.position) < 0.6f)
                {
                    _transform.gameObject.GetComponent<Ghost.GhostMovementMultiplayer>().KillGhostBySuck();
